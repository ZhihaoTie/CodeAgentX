...
我们开发 CodeAgent-X，是为了将代码任务可靠地转化为带测试证据、人工审核和审计记录的代码变更。

我们不把它定位成 Cursor 或 Codex 的聊天界面替代品。CodeAgent-X 的职责是在模型编辑代码之外补齐工程流程：任务进入、仓库隔离、工具权限、测试验证、失败重试、人工审核、PR 发布和 CI 状态回写。
...
我们当前以单机或单服务器的可靠闭环为目标。以下内容不属于 MVP：
...
MVP 主线已经冻结。后续我们优先处理缺陷、易用性、安全、文档和发布维护。
