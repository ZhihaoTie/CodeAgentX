...
Spring Boot control-plane compiles and passes 49 Maven tests on JDK 17, covering workflow state transitions, JPA persistence, GitHub issue/webhook parsing, CI webhook idempotency, runtime submit retry, timeout recovery, and local Git publishing helpers.
